...
class DAG:
    def __init__(
            ...
            # Lambda=None,
            lambdas,
            isAcyclic_n_nodes=None
    ):
        self.dataset = dataset
        if self.dataset=='MUTAG':
            self.input_dim = 7
        elif self.dataset == 'isAcyclic':
            self.input_dim = 3
        elif self.dataset=='highschool':
            self.input_dim=2
        self.model = model
        if dataset=='isAcyclic':
            self.gSpan_output_file = 'data/'+self.dataset+'/'+str(isAcyclic_n_nodes)+'_nodes_gSpan_output'
            self.isAcyclic_n_nodes = isAcyclic_n_nodes
        else:
            self.gSpan_output_file = 'data/'+self.dataset+'/gSpan_output'
    ...
    def getGNNscore(self):
        if self.dataset=='highschool':
            ...
            print('GNN score got from the model')
            return score, size
    ...
    def getSavePath(self):
        ...
        save_path = 'result/'+self.dataset+'/'+self.model._get_name()+'_result/'
